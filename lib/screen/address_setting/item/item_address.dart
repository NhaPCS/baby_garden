...
class ItemAddress extends StatefulWidget {
  ...
  const ItemAddress(
      {Key key, this.address, this.isDefault = false, this.onDeleteAddress})
      : super(key: key);
...
class _ItemAddressState extends State<ItemAddress> {
  ...
  void initState() {
    super.initState();
    addressFieldCtrl.text = widget.address.address;
  }
  ...
  void editAddress() async {
    ...
    if (newAdd == widget.address.address) {
      setState(() {
        isEditting = false;
      });
      return;
    ...
    postAddAddress(context,
            address: newAdd, isMain: 0, addressId: widget.address.id)
        .then((value) {
  ...
  void deleteAddr() async {
    deleteAddress(context, addressId: widget.address.id).then((value) {
      if (value == null) return;
      widget.onDeleteAddress(widget.address.id);
      // _getListAddressProvider.address.removeWhere((element) => element['id'] == widget.address.id);
      setState(() {
        isEditting = false;
      });
    });
